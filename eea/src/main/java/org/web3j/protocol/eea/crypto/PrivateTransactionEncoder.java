...
public class PrivateTransactionEncoder {
    ...
    public static byte[] signMessage(final RawPrivateTransaction rawTransaction, final Credentials credentials) {
    ...
    public static byte[] signMessage(
            final RawPrivateTransaction rawTransaction, final byte chainId, final Credentials credentials) {
        final byte[] encodedTransaction = encode(rawTransaction, chainId);
        final Sign.SignatureData signatureData = Sign.signMessage(
                encodedTransaction, credentials.getEcKeyPair());

        final Sign.SignatureData eip155SignatureData = createEip155SignatureData(signatureData, chainId);
    ...
    public static byte[] encode(final RawPrivateTransaction rawTransaction, final byte chainId) {
        final Sign.SignatureData signatureData = new Sign.SignatureData(
                chainId, new byte[] {}, new byte[] {});
        return encode(rawTransaction, signatureData);
    }

    private static byte[] encode(final RawPrivateTransaction rawTransaction, final Sign.SignatureData signatureData) {
