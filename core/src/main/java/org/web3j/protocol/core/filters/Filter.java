...
public abstract class Filter<T> {
	...
	public void run(ScheduledExecutorService scheduledExecutorService, long blockTime) {
		try {
			 ...
			 * occasions users have reported socket timeout exceptions when connected over
			 * HTTP to Geth and Parity clients. For examples, refer to
			 ...
			 * notified of downstream connection issues. But given the intermittent nature
			 * of the connection issues, this seems like a reasonable compromise.
			 * 
			 * The alternative approach would be to have another thread that blocks waiting
			 * on schedule.get(), catching any Exceptions thrown, and passing them back up
			 * to the caller. However, the user would then be required to recreate
			 * subscriptions manually which isn't ideal given the aforementioned issues.
			 */
	...
	/**
	 * Retrieves historic filters for the filter with the given id. Getting historic
	 * logs is not supported by all filters. If not the method should return an
	 * empty EthLog object
	 *
	 * @param filterId
